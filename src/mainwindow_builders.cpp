    ...
    for(int r=0;r<sala->getRanduri();r++){
        ...
        for(int c=0;c<cols;c++){
            ...
            if(isSel){fill=QColor("#0c3d5c");border=QColor(C_BLUE);txtC=QColor(C_BLUE);lbl="★";}
                ...
                switch(cat){case CategorieLocSeat::VIP:fill=QColor("#4a380a");border=QColor(C_GOLD);txtC=QColor(C_GOLD);break;
                ...
                switch(cat){case CategorieLocSeat::VIP:fill=QColor("#2e1f08");border=QColor("#b07818");txtC=QColor(C_GOLD);lbl="V";break;
...
// ═══ StepBar ════════════════════════════════════════════════════════════════
const QStringList StepBar::STEPS={"Film","Sală","Dată","Oră","Locuri","Food","Confirmare"};
...
void StepBar::paintEvent(QPaintEvent*){
    ...
    for(int i=0;i<n;i++){
        ...
        QColor cc;int cr;
        if(i<current_){cc=QColor(C_GRN);cr=8;}else if(i==current_){cc=QColor(C_RED);cr=10;}else{cc=QColor(C_BOR);cr=8;}
...
LocationCard::LocationCard(const CinemaLocation& loc,bool isCurrentCity,QWidget* parent)
    ...
    setFixedHeight(100);setCursor(Qt::PointingHandCursor);
    ...
    if(isCurrentCity){
        ...
        badge->setStyleSheet("background:#166534;color:#4ade80;border-radius:6px;padding:3px 10px;font-size:11px;font-weight:700;background:transparent;border:none;");
        badge->setAttribute(Qt::WA_TransparentForMouseEvents);
        hb->addWidget(badge,0,Qt::AlignRight|Qt::AlignVCenter);
    }
}
...
void MainWindow::goTo(Page p){
    ...
    else if(p==P_RESERVE_SALA)                        stepBar->setStep(1);
    else if(p==P_RESERVE_DATA)                        stepBar->setStep(2);
    else if(p==P_RESERVE_ORA)                         stepBar->setStep(3);
    else if(p==P_RESERVE_LOC)                         stepBar->setStep(4);
    else if(p==P_RESERVE_FOOD)                        stepBar->setStep(5);
    else if(p==P_RESERVE_CONFIRM)                     stepBar->setStep(6);
...
MainWindow::MainWindow(QWidget* parent):QMainWindow(parent){
    setWindowTitle("Cinema Booking System");setMinimumSize(980,700);
    QPalette pal;
    ...
    stack->addWidget(buildLoginPage());          // 0  P_LOGIN
    stack->addWidget(buildLocationPage());       // 1  P_LOCATION
    stack->addWidget(buildMainPage());           // 2  P_MAIN
    stack->addWidget(buildReserveFilmPage());    // 3  P_RESERVE_FILM
    stack->addWidget(buildReserveSalaPage());    // 4  P_RESERVE_SALA
    stack->addWidget(buildReserveDataPage());    // 5  P_RESERVE_DATA
    stack->addWidget(buildReserveOraPage());     // 6  P_RESERVE_ORA
    stack->addWidget(buildReserveLocPage());     // 7  P_RESERVE_LOC
    stack->addWidget(buildReserveFoodPage());    // 8  P_RESERVE_FOOD
    stack->addWidget(buildReserveConfirmPage()); // 9  P_RESERVE_CONFIRM
    stack->addWidget(buildBrowsePage());         // 10 P_BROWSE
    stack->addWidget(buildMapPage());            // 11 P_MAP
    stack->addWidget(buildCancelPage());         // 12 P_CANCEL
    stack->addWidget(buildProfilePage());        // 13 P_PROFILE
    stack->addWidget(buildAdminPage());          // 14 P_ADMIN
    stack->addWidget(buildFilmDetailPage());     // 15 P_FILM_DETAIL
    stack->addWidget(buildConfirmDonePage());    // 16 P_CONFIRM_DONE
...
QWidget* MainWindow::buildNavbar(){
    ...
    hb->addWidget(logo);hb->addSpacing(16);
...
QWidget* MainWindow::buildLoginPage(){
    ...
    auto* cs=new QLabel("Intră în cont pentru a face rezervări");cs->setStyleSheet("font-size:12px;color:" C_MUT ";margin-bottom:10px;");fl->addWidget(cs);
...
QWidget* MainWindow::buildFilmDetailPage(){
    ...
    connect(bTrailer,&QPushButton::clicked,this,[this]{
        if(rezFilm) QDesktopServices::openUrl(QUrl("https://www.youtube.com/results?search_query="+
            QUrl::toPercentEncoding(QString::fromStdString(rezFilm->getTitlu())+" trailer official")));
    });
...
QWidget* MainWindow::buildReserveSalaPage(){
    QVBoxLayout* vb;auto* w=pageWrap(vb);
    auto* bk=ghostBtn("← Înapoi",36);addHeader(vb,"Alege Sala","Selectează sala preferată",bk);
...
QWidget* MainWindow::buildReserveDataPage(){
    ...
    connect(bk,&QPushButton::clicked,this,[this]{goTo(P_RESERVE_SALA);});return w;
}

// ═══ Reserve – Oră ════════════════════════════════════════════════════════════
QWidget* MainWindow::buildReserveOraPage(){
    QVBoxLayout* vb;auto* w=pageWrap(vb);
    auto* bk=ghostBtn("← Înapoi",36);addHeader(vb,"Alege Ora","Selectează sesiunea dorită",bk);
    ...
    connect(bk,&QPushButton::clicked,this,[this]{goTo(P_RESERVE_DATA);});return w;
}

// ═══ Reserve – Locuri (group booking) ═════════════════════════════════════════
QWidget* MainWindow::buildReserveLocPage(){
    ...
    vb->addWidget(filmMiniCard(miniLoc));vb->addSpacing(10);
    ...
    connect(bCont,&QPushButton::clicked,this,[this]{
        if(rezLocuri.empty()){QMessageBox::warning(this,"","Selectează cel puțin un loc!");return;}
        // build confirm summary
        Sala* sala=cinema.getSalaById(rezSalaId);
        if(!sala){QMessageBox::warning(this,"","Eroare sală!");return;}
        // go to food page
        rezFood=FoodOrder{};
        refreshFoodPage();
        goTo(P_RESERVE_FOOD);
...
QWidget* MainWindow::buildReserveFoodPage(){
    ...
    vb->addWidget(filmMiniCard(miniFood));vb->addSpacing(12);
...
QWidget* MainWindow::buildProfilePage(){
    ...
    connect(bPw,&QPushButton::clicked,this,[this]{
        ...
        QString nw=QInputDialog::getText(this,"Parolă nouă","Parola nouă:",QLineEdit::Password,"",&ok);if(!ok||nw.isEmpty())return;
        QString cf=QInputDialog::getText(this,"Confirmare","Confirmă parola:",QLineEdit::Password,"",&ok);if(!ok||nw!=cf){QMessageBox::warning(this,"","Parolele nu coincid!");return;}
        cinema.schimbaParola(u->username,nw.toStdString());u->parola=nw.toStdString();QMessageBox::information(this,"","Parolă schimbată!");
    });
    connect(bk,&QPushButton::clicked,this,[this]{goTo(P_MAIN);});return w;
}
...
QWidget* MainWindow::buildAdminPage(){
    ...
    ob->addWidget(bAO);ob->addWidget(bDO);ob->addStretch();oV->addLayout(ob);
    adminTabs->addTab(ot,"🕐  Ore");
...
QWidget* MainWindow::buildConfirmDonePage(){
    ...
    vb->addWidget(bHome,0,Qt::AlignHCenter);return w;
}
