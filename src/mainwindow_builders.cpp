...
static QLabel* secLbl(const QString& t){
    auto* l=new QLabel(t);
    l->setStyleSheet("font-size:11px;font-weight:700;color:" C_DIM ";letter-spacing:2px;");
    return l;
}
static QFrame* hLine(){
...
static QString ratingStars(double r){
    int s=qRound(r/2.0); QString st;
    for(int i=0;i<s;i++) st+="★";
    for(int i=s;i<5;i++) st+="☆";
    return st;
}
...
void SeatMapWidget::load(Sala* s,const Data& d,const std::string& o){sala=s;data=d;ora=o;selRow=selCol=hoverRow=hoverCol=-1;updateGeometry();update();}
void SeatMapWidget::setSelected(int r,int c){selRow=r;selCol=c;update();}
...
void SeatMapWidget::mousePressEvent(QMouseEvent* e){if(!readOnly){auto[r,c]=seatAt(e->pos());if(r>=0)emit seatClicked(r,c);}}
...
void SeatMapWidget::paintEvent(QPaintEvent*){
    ...
    int sx1=ML-12,sx2=ML+cols*(S+G)-G+12,sy=MT-50;
    QPainterPath arc; arc.moveTo(sx1,sy+28); arc.quadTo((sx1+sx2)/2,sy,sx2,sy+28);
    ...
    p.setPen(QColor(C_DIM)); p.setFont(QFont("Arial",9,QFont::Bold));
    p.drawText(QRect(sx1,sy+28,sx2-sx1,18),Qt::AlignHCenter,"— ECRAN —");
    p.setPen(QColor(C_DIM)); p.setFont(QFont("Segoe UI",8));
    for(int c=0;c<cols;c++){QRect sr=seatRect(0,c);p.drawText(QRect(sr.x(),MT-22,S,18),Qt::AlignHCenter|Qt::AlignVCenter,QString::number(c+1));}
    for(int r=0;r<sala->getRanduri();r++){
        QRect first=seatRect(r,0);
        p.setPen(QColor(C_DIM)); p.setFont(QFont("Segoe UI",8));
        p.drawText(QRect(0,first.y(),ML-10,S),Qt::AlignRight|Qt::AlignVCenter,QString("R%1").arg(r+1));
        for(int c=0;c<cols;c++){
            bool occ=sala->esteOcupat(data,ora,r,c);
            CategorieLocSeat cat=sala->getCategorieLocSeat(r,c);
            bool sel=(r==selRow&&c==selCol),hov=(r==hoverRow&&c==hoverCol)&&!readOnly;
            QColor fill,border,txtC; QString lbl;
            if(sel){fill=QColor("#0c3d5c");border=QColor(C_BLUE);txtC=QColor(C_BLUE);lbl="★";}
            else if(occ){fill=QColor("#161620");border=QColor("#252535");txtC=QColor("#3a3a55");lbl="✕";}
            else if(hov){switch(cat){case CategorieLocSeat::VIP:fill=QColor("#4a380a");border=QColor(C_GOLD);txtC=QColor(C_GOLD);break;case CategorieLocSeat::CUPLU:fill=QColor("#321866");border=QColor(C_PURP);txtC=QColor(C_PURP);break;default:fill=QColor("#133a22");border=QColor(C_GRN);txtC=QColor(C_GRN);break;}}
            else{switch(cat){case CategorieLocSeat::VIP:fill=QColor("#2e1f08");border=QColor("#b07818");txtC=QColor(C_GOLD);lbl="V";break;case CategorieLocSeat::CUPLU:fill=QColor("#200e42");border=QColor("#5b21b6");txtC=QColor(C_PURP);lbl="C";break;default:fill=QColor("#0d2618");border=QColor("#166534");txtC=QColor(C_GRN);break;}}
            QRect sr=seatRect(r,c);
            p.setBrush(fill); p.setPen(QPen(border,sel?1.8:1.2)); p.drawRoundedRect(sr.adjusted(1,1,-1,-1),5,5);
            if(!lbl.isEmpty()){p.setPen(txtC);p.setFont(QFont("Segoe UI",9,sel?QFont::Bold:QFont::Normal));p.drawText(sr,Qt::AlignCenter,lbl);}
        }
    }
}
...
// MenuCard
// ══════════════════════════════════════════════════════════════════════════════
MenuCard::MenuCard(const QString& emoji,const QString& title,const QString& desc,bool gold,QWidget* parent)
    :QFrame(parent)
{
    setMinimumSize(180,120); setMaximumSize(380,260); setSizePolicy(QSizePolicy::Expanding,QSizePolicy::Expanding);
    setCursor(Qt::PointingHandCursor);
    if(gold){nor_="QFrame{background:#16120a;border:1.5px solid #3a2a0a;border-radius:14px;}";
             hov_="QFrame{background:#1e180d;border:2px solid " C_GOLD ";border-radius:14px;}";}
    else    {nor_="QFrame{background:" C_CARD ";border:1.5px solid " C_BOR2 ";border-radius:14px;}";
             hov_="QFrame{background:" C_RAISE ";border:2px solid " C_RED ";border-radius:14px;}";}
    setStyleSheet(nor_);
    ...
    vb->addWidget(mk(emoji,"font-size:30px;background:transparent;border:none;"));
...
// StepBar
// ══════════════════════════════════════════════════════════════════════════════
const QStringList StepBar::STEPS={"Film","Sală","Dată","Oră","Loc","Confirmare"};
...
void StepBar::paintEvent(QPaintEvent*){
    ...
    int n=STEPS.size(), w=width(), cy=height()/2-4;
    int spc=w/n;
    for(int i=0;i<n;i++){
        int x=spc/2+i*spc;
        if(i<n-1){
            int nx=spc/2+(i+1)*spc;
            p.setPen(QPen(i<current_?QColor(C_GRN):QColor(C_BOR),2));
            p.drawLine(x+11,cy,nx-11,cy);
        }
        QColor cc; int cr;
...
QFrame* MainWindow::filmMiniCard(QLabel*& lbl){
    ...
    lbl=new QLabel("Alege un film pentru a continua");
    lbl->setStyleSheet("color:" C_MUT ";font-size:13px;background:transparent;border:none;");
    hb->addWidget(ico); hb->addWidget(lbl,1); return f;
}

// ══════════════════════════════════════════════════════════════════════════════
// goTo  –  with fade + step bar
// ══════════════════════════════════════════════════════════════════════════════
void MainWindow::goTo(Page p){
    navbar->setVisible(p!=P_LOGIN);
    bool inRes=(p==P_FILM_DETAIL)||(p>=P_RESERVE_SALA&&p<=P_RESERVE_CONFIRM);
    stepBar->setVisible(inRes);
    if(p==P_FILM_DETAIL)                                   stepBar->setStep(0);
    else if(p>=P_RESERVE_SALA&&p<=P_RESERVE_CONFIRM)       stepBar->setStep(p-P_RESERVE_SALA+1);
...
MainWindow::MainWindow(QWidget* parent):QMainWindow(parent){
    setWindowTitle("Cinema Booking System"); setMinimumSize(960,680);
    QPalette pal;
    ...
    cinema.initConturi(); cinema.incarcaFilme("data/filme.txt");
    ...
    stack->addWidget(buildMainPage());        // 1
    stack->addWidget(buildReserveFilmPage()); // 2
    stack->addWidget(buildReserveSalaPage()); // 3
    stack->addWidget(buildReserveDataPage()); // 4
    stack->addWidget(buildReserveOraPage());  // 5
    stack->addWidget(buildReserveLocPage());  // 6
    stack->addWidget(buildReserveConfirmPage());// 7
    stack->addWidget(buildBrowsePage());      // 8
    stack->addWidget(buildMapPage());         // 9
    stack->addWidget(buildCancelPage());      // 10
    stack->addWidget(buildProfilePage());     // 11
    stack->addWidget(buildAdminPage());       // 12
    stack->addWidget(buildFilmDetailPage());  // 13
    stack->addWidget(buildConfirmDonePage()); // 14
...
QWidget* MainWindow::buildNavbar(){
    ...
    auto* logo=new QLabel; logo->setTextFormat(Qt::RichText);
    logo->setText("<span style='font-size:18px;'>🎬</span>"
                  "<span style='font-size:17px;font-weight:800;color:#e50914;letter-spacing:3px;'> CINEMA</span>");
    hb->addWidget(logo); hb->addStretch();
...
QWidget* MainWindow::buildLoginPage(){
    ...
    vb->addWidget(mk("🎬","font-size:52px;"));
    vb->addWidget(mk("CINEMA","font-size:34px;font-weight:800;color:" C_RED ";letter-spacing:7px;margin-bottom:4px;"));
    ...
    card->setStyleSheet("QFrame{background:" C_CARD ";border-radius:18px;border:1px solid " C_BOR2 ";}");
    auto* fl=new QVBoxLayout(card); fl->setContentsMargins(38,36,38,36); fl->setSpacing(14);
    auto* ct=new QLabel("Bun venit înapoi"); ct->setStyleSheet("font-size:21px;font-weight:700;color:" C_TXT ";margin-bottom:6px;"); fl->addWidget(ct);
    auto* cs=new QLabel("Intră în contul tău pentru a face rezervări"); cs->setStyleSheet("font-size:12px;color:" C_MUT ";margin-bottom:10px;"); fl->addWidget(cs);
    ...
    auto* bR=ghostBtn("Creează cont nou",46); fl->addWidget(bR);
    ...
    connect(bR,&QPushButton::clicked,this,&MainWindow::onRegister);
    return w;
}

// ══════════════════════════════════════════════════════════════════════════════
// Main menu  –  hero banner + action cards + upcoming strip
// ══════════════════════════════════════════════════════════════════════════════
QWidget* MainWindow::buildMainPage(){
    ...
    // ── Hero banner ──────────────────────────────────────────────────────────
    heroFrame=new QFrame;
    heroFrame->setFixedHeight(162);
    ...
    lblHeroTitle=new QLabel("—");
    lblHeroTitle->setStyleSheet("font-size:26px;font-weight:800;color:white;"); lblHeroTitle->setWordWrap(true);
    hLeft->addWidget(lblHeroTitle);
    lblHeroInfo=new QLabel("—");
    lblHeroInfo->setStyleSheet("font-size:13px;color:" C_MUT ";"); hLeft->addWidget(lblHeroInfo);
    hLeft->addStretch();
    ...
    bHero->setStyleSheet("QPushButton{background:" C_RED ";color:white;border:none;border-radius:7px;font-size:13px;font-weight:700;}"
                         "QPushButton:hover{background:" C_RHOV ";}");
    ...
    struct E{QString e,t,d;void(MainWindow::*s)();};
    std::vector<E> entries={
        {"🎟","Rezervare Nouă","Alege film, sală și loc",&MainWindow::onNewReservation},
        {"🎭","Browse Genuri","Descoperă după gen",&MainWindow::onBrowseGenuri},
        {"🗺","Hartă Sală","Verifică disponibilitatea",&MainWindow::onHartaSala},
        {"❌","Anulare","Anulează o rezervare",&MainWindow::onAnulareRezervare},
        {"👤","Profilul Meu","Istoric și setări",&MainWindow::onProfilMeu},
    };
    int col=0,row=0;
    for(auto& e:entries){auto*c=new MenuCard(e.e,e.t,e.d,false,w);connect(c,&MenuCard::clicked,this,e.s);grid->addWidget(c,row,col);if(++col==3){col=0;row++;}}
    cardAdmin=new MenuCard("⚙️","Panou Admin","Gestionare sistem",true,w);
    connect(cardAdmin,&MenuCard::clicked,this,&MainWindow::onAdminPanel);
    cardAdmin->hide(); grid->addWidget(cardAdmin,row,col);
    vb->addLayout(grid,1);

    // ── Upcoming films strip ──────────────────────────────────────────────────
...
QWidget* MainWindow::buildFilmDetailPage(){
    ...
    // header built manually below

    // re-build header row properly
    {
        // Remove the placeholder added by addHeader and redo
        // Actually let's just build from scratch without addHeader
    }
    // Clear and rebuild
    while(vb->count()) delete vb->takeAt(0);
    vb->setContentsMargins(40,24,40,28); vb->setSpacing(0);
    ...
    detailTitle->setStyleSheet("font-size:28px;font-weight:800;color:" C_TXT ";");
    detailTitle->setWordWrap(true); vb->addWidget(detailTitle);

    detailGenreRating=new QLabel;
    detailGenreRating->setStyleSheet("font-size:13px;color:" C_MUT ";margin-bottom:12px;");
    vb->addWidget(detailGenreRating);
    vb->addWidget(hLine()); vb->addSpacing(16);

    // Two-column: info left, poster right
    ...
    detailDesc->setStyleSheet("font-size:14px;color:#c8c8d8;line-height:1.7;");
    detailDesc->setTextFormat(Qt::PlainText);
    leftVb->addWidget(detailDesc);

    detailMeta=new QLabel;
    detailMeta->setTextFormat(Qt::RichText);
    detailMeta->setStyleSheet("font-size:13px;color:" C_TXT ";line-height:1.9;"
                              "background:" C_CARD ";border:1px solid " C_BOR2 ";"
                              "border-radius:12px;padding:18px 22px;");
    leftVb->addWidget(detailMeta);

    // price + book
    auto* bookRow=new QHBoxLayout; bookRow->setSpacing(16);
    detailPrice=new QLabel;
    detailPrice->setStyleSheet("font-size:22px;font-weight:800;color:" C_RED ";");
    bookRow->addWidget(detailPrice);
    btnDetailBook=redBtn("🎟  Rezervă acum",50); btnDetailBook->setFixedWidth(200);
    connect(btnDetailBook,&QPushButton::clicked,this,&MainWindow::onProceedFromDetail);
    bookRow->addWidget(btnDetailBook);
    detailUnavail=new QLabel;
    detailUnavail->setStyleSheet("font-size:13px;color:" C_GOLD ";");
    bookRow->addWidget(detailUnavail); bookRow->addStretch();
    leftVb->addLayout(bookRow);
    split->addLayout(leftVb,3);

    // Right: poster
    auto* rightVb=new QVBoxLayout; rightVb->setAlignment(Qt::AlignTop|Qt::AlignHCenter);
    detailPoster=new QLabel;
    detailPoster->setFixedSize(220,330);
    detailPoster->setAlignment(Qt::AlignCenter);
    detailPoster->setStyleSheet("background:" C_CARD ";border:1px solid " C_BOR2 ";"
                                "border-radius:12px;font-size:48px;color:" C_DIM ";");
    rightVb->addWidget(detailPoster);
    split->addLayout(rightVb,1);

    auto* scrollArea=new QScrollArea; scrollArea->setWidgetResizable(true);
    scrollArea->setStyleSheet("QScrollArea{background:" C_BG ";border:none;}");
    auto* inner=new QWidget; inner->setStyleSheet("background:" C_BG ";");
    auto* innerVb=new QVBoxLayout(inner);
    innerVb->setContentsMargins(0,0,0,0); innerVb->addLayout(split); innerVb->addStretch();
    scrollArea->setWidget(inner);
    vb->addWidget(scrollArea,1);
    return w;
}

// ══════════════════════════════════════════════════════════════════════════════
// Reserve – Film search  (with genre + 3D filters)
// ══════════════════════════════════════════════════════════════════════════════
QWidget* MainWindow::buildReserveFilmPage(){
    QVBoxLayout* vb; auto* w=pageWrap(vb);
    auto* bk=ghostBtn("← Înapoi",36);
    addHeader(vb,"Alege Filmul","Caută sau filtrează titluri",bk);

    // Filter row
    auto* fRow=new QHBoxLayout; fRow->setSpacing(10);
    leSearch=new QLineEdit; leSearch->setPlaceholderText("🔍  Caută titlu…");
    leSearch->setStyleSheet(inputSS()); leSearch->setFixedHeight(42);
    ...
    lblFilmsEmpty=new QLabel("Niciun film găsit");
    lblFilmsEmpty->setStyleSheet("font-size:16px;font-weight:600;color:" C_MUT ";margin-top:8px;");
    lblFilmsEmpty->setAlignment(Qt::AlignCenter);
    auto* eSub=new QLabel("Încearcă să modifici filtrele sau termenul de căutare");
    eSub->setStyleSheet("font-size:12px;color:" C_DIM ";"); eSub->setAlignment(Qt::AlignCenter);
    eVb->addWidget(eIco); eVb->addWidget(lblFilmsEmpty); eVb->addWidget(eSub);
    filmListStack->addWidget(emptyW);
    vb->addWidget(filmListStack,1);

    auto* btmRow=new QHBoxLayout; btmRow->addStretch();
    auto* bGo=redBtn("Continuă  →"); bGo->setFixedWidth(160);
    btmRow->addWidget(bGo); vb->addSpacing(14); vb->addLayout(btmRow);
    ...
    connect(btnFilm3D,&QPushButton::clicked,this,[this]{
        filmFilter3D=(filmFilter3D+2)%3-1;
        btnFilm3D->setText(filmFilter3D==-1?"Format: Toate":filmFilter3D==0?"Format: 2D":"Format: 3D");
...
// Reserve – Sală
// ══════════════════════════════════════════════════════════════════════════════
QWidget* MainWindow::buildReserveSalaPage(){
    QVBoxLayout* vb; auto* w=pageWrap(vb);
    auto* bk=ghostBtn("← Înapoi",36);
    addHeader(vb,"Alege Sala","Selectează sala preferată",bk);
    vb->addWidget(filmMiniCard(miniSala)); vb->addSpacing(12);
    lwSali=new QListWidget; lwSali->setStyleSheet(listSS()); lwSali->setFont(QFont("Segoe UI",14)); lwSali->setSpacing(2);
    vb->addWidget(lwSali,1);
    auto* row=new QHBoxLayout; row->addStretch();
    auto* bGo=redBtn("Continuă  →"); bGo->setFixedWidth(160); row->addWidget(bGo);
    vb->addSpacing(14); vb->addLayout(row);
    connect(bGo,&QPushButton::clicked,this,&MainWindow::onSalaSelected);
    connect(lwSali,&QListWidget::itemDoubleClicked,this,&MainWindow::onSalaSelected);
    connect(bk,&QPushButton::clicked,this,[this]{goTo(P_FILM_DETAIL);});
    return w;
}
...
QWidget* MainWindow::buildReserveDataPage(){
    QVBoxLayout* vb; auto* w=pageWrap(vb);
    auto* bk=ghostBtn("← Înapoi",36);
    addHeader(vb,"Alege Data","Disponibilitate pentru 7 zile",bk);
    vb->addWidget(filmMiniCard(miniData)); vb->addSpacing(12);
    ...
    connect(bk,&QPushButton::clicked,this,[this]{goTo(P_RESERVE_SALA);});
    return w;
}
...
// Reserve – Ora
// ══════════════════════════════════════════════════════════════════════════════
QWidget* MainWindow::buildReserveOraPage(){
    QVBoxLayout* vb; auto* w=pageWrap(vb);
    auto* bk=ghostBtn("← Înapoi",36);
    addHeader(vb,"Alege Ora","Selectează sesiunea dorită",bk);
    vb->addWidget(filmMiniCard(miniOra)); vb->addSpacing(12);
    lwOra=new QListWidget; lwOra->setStyleSheet(listSS()); lwOra->setFont(QFont("Segoe UI",14)); lwOra->setSpacing(2);
    vb->addWidget(lwOra,1);
    auto* row=new QHBoxLayout; row->addStretch();
    auto* bGo=redBtn("Continuă  →"); bGo->setFixedWidth(160); row->addWidget(bGo);
    vb->addSpacing(14); vb->addLayout(row);
    connect(bGo,&QPushButton::clicked,this,&MainWindow::onOraSelected);
    connect(lwOra,&QListWidget::itemDoubleClicked,this,&MainWindow::onOraSelected);
    connect(bk,&QPushButton::clicked,this,[this]{goTo(P_RESERVE_DATA);});
    return w;
}

// ══════════════════════════════════════════════════════════════════════════════
// Reserve – Loc (seat map)
// ══════════════════════════════════════════════════════════════════════════════
QWidget* MainWindow::buildReserveLocPage(){
    QVBoxLayout* vb; auto* w=pageWrap(vb);
    auto* bk=ghostBtn("← Înapoi",36);
    addHeader(vb,"Alege Locul","Apasă pe un loc liber din hartă",bk);
    vb->addWidget(filmMiniCard(miniLoc)); vb->addSpacing(12);
    ...
    legRow->addStretch();
    vb->addLayout(legRow); vb->addSpacing(12);
    ...
    lblSeatInfo=new QLabel("Selectează un loc liber");
    lblSeatInfo->setStyleSheet("color:" C_MUT ";font-size:13px;margin-top:10px;"); vb->addWidget(lblSeatInfo);

    auto* btm=new QHBoxLayout; btm->addStretch();
    auto* bCont=redBtn("Confirmă Locul  →"); bCont->setFixedWidth(210); btm->addWidget(bCont);
    ...
    connect(bCont,&QPushButton::clicked,this,[this]{
        if(rezRow<0){QMessageBox::warning(this,"","Selectează un loc!");return;}
        Sala* sala=cinema.getSalaById(rezSalaId);
        if(!sala||sala->esteOcupat(rezData,rezOra,rezRow,rezCol)){QMessageBox::warning(this,"","Locul este ocupat!");return;}
        CategorieLocSeat cat=sala->getCategorieLocSeat(rezRow,rezCol);
        ...
        oss<<"<table style='font-size:14px;line-height:2;'>"
           <<"<tr><td style='color:"<<C_MUT<<";width:80px;'>Film</td><td><b>"<<rezFilm->getTitlu()<<"</b></td></tr>"
           <<"<tr><td style='color:"<<C_MUT<<";'>Sală</td><td>"<<sala->getNume()<<"</td></tr>"
           <<"<tr><td style='color:"<<C_MUT<<";'>Data</td><td>"<<rezData.toString()<<"</td></tr>"
           <<"<tr><td style='color:"<<C_MUT<<";'>Ora</td><td>"<<rezOra<<"</td></tr>"
           <<"<tr><td style='color:"<<C_MUT<<";'>Loc</td><td>R"<<rezRow+1<<" L"<<rezCol+1
           <<"  <span style='color:"<<C_DIM<<";'>["<<numeCategorie(cat)<<"]</span></td></tr>"
           <<"<tr><td style='color:"<<C_MUT<<";'>Preț</td><td style='color:#e50914;font-size:18px;font-weight:800;'>"
           <<std::fixed<<std::setprecision(2)<<pret<<" RON</td></tr></table>";
        lblConfirmDetails->setText(QString::fromStdString(oss.str()));
        // generate booking reference
        const char ch[]="ABCDEFGHJKLMNPQRSTUVWXYZ23456789";
        std::string ref="CIN-";
        for(int i=0;i<8;i++) ref+=ch[rand()%32];
        lblBookingRef->setText("Referință rezervare:  <b style='color:" C_BLUE ";font-size:16px;'>"
                               +QString::fromStdString(ref)+"</b>");
        leEmail->clear(); goTo(P_RESERVE_CONFIRM);
    });
    connect(bk,&QPushButton::clicked,this,[this]{goTo(P_RESERVE_ORA);});
    return w;
}
...
QWidget* MainWindow::buildReserveConfirmPage(){
    QVBoxLayout* vb; auto* w=pageWrap(vb);
    auto* bk=ghostBtn("← Înapoi",36);
    addHeader(vb,"Sumar Rezervare","Verifică detaliile și confirmă",bk);
    vb->addWidget(filmMiniCard(miniConf)); vb->addSpacing(12);

    lblConfirmDetails=new QLabel; lblConfirmDetails->setTextFormat(Qt::RichText);
    lblConfirmDetails->setStyleSheet("background:" C_CARD ";color:" C_TXT ";"
        "border:1px solid " C_BOR2 ";border-radius:14px;padding:28px 32px;");
    vb->addWidget(lblConfirmDetails);

    lblBookingRef=new QLabel; lblBookingRef->setTextFormat(Qt::RichText);
    lblBookingRef->setStyleSheet("font-size:13px;color:" C_MUT ";margin-top:8px;");
    vb->addWidget(lblBookingRef); vb->addSpacing(16);

    vb->addWidget(secLbl("EMAIL (OPȚIONAL – PENTRU CONFIRMARE)")); vb->addSpacing(6);
    leEmail=new QLineEdit; leEmail->setPlaceholderText("adresa@exemplu.com");
    leEmail->setStyleSheet(inputSS()); leEmail->setFixedHeight(46); vb->addWidget(leEmail);
    vb->addStretch();

    auto* row=new QHBoxLayout; row->addStretch();
    auto* bConf=new QPushButton("✔   Confirmă Rezervarea"); bConf->setFixedHeight(50); bConf->setFixedWidth(240);
    bConf->setCursor(Qt::PointingHandCursor);
    bConf->setStyleSheet("QPushButton{background:#16a34a;color:white;border:none;border-radius:10px;"
                         "font-size:14px;font-weight:700;}"
                         "QPushButton:hover{background:#15803d;}");
    row->addWidget(bConf); vb->addLayout(row);
    connect(bConf,&QPushButton::clicked,this,&MainWindow::onConfirmReservation);
    connect(bk,&QPushButton::clicked,this,[this]{goTo(P_RESERVE_LOC);});
    return w;
}
...
QWidget* MainWindow::buildBrowsePage(){
    QVBoxLayout* vb; auto* w=pageWrap(vb);
    auto* bk=ghostBtn("← Înapoi",36);
    addHeader(vb,"Browse după Gen","Explorează filmele din colecție",bk);
    ...
    auto* beLbl=new QLabel("Niciun film în acest gen"); beLbl->setStyleSheet("font-size:15px;font-weight:600;color:" C_MUT ";margin-top:8px;"); beLbl->setAlignment(Qt::AlignCenter);
    ...
    // connect list count to stack switcher
    connect(lwBrowseFilms->model(),&QAbstractItemModel::rowsInserted,this,[browseListStack](){ browseListStack->setCurrentIndex(0); });
    connect(lwBrowseFilms->model(),&QAbstractItemModel::rowsRemoved,this,[browseListStack,this](){ if(lwBrowseFilms->count()==0) browseListStack->setCurrentIndex(1); });
    split->addWidget(browseListStack,1);
    auto* dc=new QFrame; dc->setFixedWidth(300);
    dc->setStyleSheet("QFrame{background:" C_CARD ";border:1px solid " C_BOR2 ";border-radius:14px;}");
    auto* dcVb=new QVBoxLayout(dc); dcVb->setContentsMargins(24,24,24,24); dcVb->setSpacing(0);
    lblBrowseDetail=new QLabel("Selectează un film");
    lblBrowseDetail->setStyleSheet("color:" C_TXT ";font-size:13px;line-height:1.8;");
    lblBrowseDetail->setTextFormat(Qt::RichText); lblBrowseDetail->setAlignment(Qt::AlignTop); lblBrowseDetail->setWordWrap(true);
    ...
    dcVb->addWidget(bRes); dcVb->addSpacing(6); dcVb->addWidget(bDet);
    split->addWidget(dc); vb->addLayout(split,1);
...
QWidget* MainWindow::buildMapPage(){
    ...
    auto* br=new QPushButton("🔄  Actualizează"); br->setFixedHeight(36);
    br->setStyleSheet("QPushButton{background:" C_RAISE ";color:" C_TXT ";border:1px solid " C_BOR ";border-radius:7px;padding:0 16px;font-size:13px;}QPushButton:hover{background:#28283a;}");
    br->setCursor(Qt::PointingHandCursor); sr->addWidget(br); sr->addStretch();
    vb->addLayout(sr); vb->addSpacing(16);
    ...
    scroll->setWidget(inner); vb->addWidget(scroll,1);
    connect(br,&QPushButton::clicked,this,&MainWindow::refreshMapPage);
    connect(bk,&QPushButton::clicked,this,[this]{goTo(P_MAIN);});
    return w;
}
...
QWidget* MainWindow::buildCancelPage(){
    ...
    sr->addWidget(comboLbl("Ora:"));  sr->addWidget(cbCancelOra);  sr->addStretch();
    vb->addLayout(sr); vb->addSpacing(16);
    auto* scroll=new QScrollArea; scroll->setWidgetResizable(true); scroll->setStyleSheet("QScrollArea{background:" C_BG ";border:none;}");
    auto* inner=new QWidget; inner->setStyleSheet("background:" C_BG ";");
    auto* iVb=new QVBoxLayout(inner); iVb->setAlignment(Qt::AlignCenter);
    ...
    scroll->setWidget(inner); vb->addWidget(scroll,1);
    auto* btm=new QHBoxLayout; btm->addStretch();
    auto* bCan=new QPushButton("✕   Anulează Rezervarea Selectată"); bCan->setFixedHeight(46);
    bCan->setCursor(Qt::PointingHandCursor);
    ...
    connect(cbCancelSala,QOverload<int>::of(&QComboBox::currentIndexChanged),this,&MainWindow::refreshCancelSeats);
    connect(cbCancelData,QOverload<int>::of(&QComboBox::currentIndexChanged),this,&MainWindow::refreshCancelSeats);
...
QWidget* MainWindow::buildProfilePage(){
    QVBoxLayout* vb; auto* w=pageWrap(vb);
    auto* bk=ghostBtn("← Înapoi",36); addHeader(vb,"Profilul Meu","Istoricul rezervărilor și setări cont",bk);
    vb->addWidget(secLbl("ISTORIC REZERVĂRI")); vb->addSpacing(8);
    teHistory=new QTextEdit; teHistory->setReadOnly(true);
    teHistory->setStyleSheet("QTextEdit{background:" C_CARD ";color:" C_TXT ";border:1px solid " C_BOR2 ";border-radius:10px;padding:14px;font-size:13px;}");
    vb->addWidget(teHistory,1); vb->addSpacing(14);
    auto* bPw=ghostBtn("🔑   Schimbă Parola",42); bPw->setFixedWidth(200); vb->addWidget(bPw,0,Qt::AlignLeft);
    connect(bPw,&QPushButton::clicked,this,[this]{
        auto*u=cinema.getCurent();if(!u)return;
        bool ok;
        QString old=QInputDialog::getText(this,"Parolă curentă","Parola actuală:",QLineEdit::Password,"",&ok);
        if(!ok||old.toStdString()!=u->parola){QMessageBox::warning(this,"","Parolă incorectă!");return;}
        QString nw=QInputDialog::getText(this,"Parolă nouă","Parola nouă:",QLineEdit::Password,"",&ok);
        if(!ok||nw.isEmpty())return;
        QString cf=QInputDialog::getText(this,"Confirmare","Confirmă parola:",QLineEdit::Password,"",&ok);
        if(!ok||nw!=cf){QMessageBox::warning(this,"","Parolele nu coincid!");return;}
        cinema.schimbaParola(u->username,nw.toStdString());u->parola=nw.toStdString();
        QMessageBox::information(this,"","Parolă schimbată!");
    });
    connect(bk,&QPushButton::clicked,this,[this]{goTo(P_MAIN);});
    return w;
}
...
QWidget* MainWindow::buildAdminPage(){
    ...
    teAllRes=new QTextEdit; teAllRes->setReadOnly(true);
    teAllRes->setStyleSheet("background:" C_BG ";color:" C_TXT ";font-family:'Consolas','Courier New';font-size:12px;border:none;padding:12px;");
    adminTabs->addTab(teAllRes,"📋  Rezervări");
    teReport=new QTextEdit; teReport->setReadOnly(true);
    teReport->setStyleSheet("background:" C_BG ";color:" C_TXT ";font-family:'Consolas','Courier New';font-size:12px;border:none;padding:12px;");
    ...
    fb->addWidget(bA);fb->addWidget(bE);fb->addWidget(bD);fb->addStretch();
    ftV->addLayout(fb); adminTabs->addTab(ft,"🎬  Filme");
    // Ore tab
    auto* ot=new QWidget; ot->setStyleSheet("background:" C_CARD ";");
    auto* oV=new QVBoxLayout(ot); oV->setContentsMargins(16,16,16,16); oV->setSpacing(10);
    lwAdminOre=new QListWidget; lwAdminOre->setStyleSheet(listSS()); oV->addWidget(lwAdminOre,1);
    auto* ob=new QHBoxLayout; ob->setSpacing(8);
    auto* bAO=redBtn("+ Adaugă Oră",38); auto* bDO=ghostBtn("🗑  Șterge",38);
    bDO->setStyleSheet(bDO->styleSheet().replace(C_MUT,"#f87171").replace(C_BOR,"#5a1a1a"));
    ob->addWidget(bAO);ob->addWidget(bDO);ob->addStretch();
    oV->addLayout(ob); adminTabs->addTab(ot,"🕐  Ore");
    // Users tab
    ...
    connect(adminTabs,&QTabWidget::currentChanged,this,[this](int i){
        ...
        else if(i==2)refreshAdminFilms();else if(i==3)refreshAdminOre();else if(i==4)refreshAdminUsers();});
    ...
    connect(bAO,&QPushButton::clicked,this,&MainWindow::onAdminAddOra);
    connect(bDO,&QPushButton::clicked,this,&MainWindow::onAdminDeleteOra);
...
// ══════════════════════════════════════════════════════════════════════════════
// Confirm Done  –  full-page booking confirmation
// ══════════════════════════════════════════════════════════════════════════════
QWidget* MainWindow::buildConfirmDonePage(){
    ...
    auto* chk=new QLabel("✓");
    chk->setFixedSize(88,88);
    chk->setAlignment(Qt::AlignCenter);
    chk->setStyleSheet("font-size:44px;font-weight:900;color:white;"
                       "background:#16a34a;border-radius:44px;");
    vb->addWidget(chk,0,Qt::AlignHCenter);
    vb->addSpacing(18);

    auto* ttl=new QLabel("Rezervare Confirmată!");
    ttl->setStyleSheet("font-size:28px;font-weight:800;color:" C_TXT ";");
    ttl->setAlignment(Qt::AlignCenter); vb->addWidget(ttl);
    vb->addSpacing(8);

    auto* sub=new QLabel("Biletul tău a fost rezervat cu succes.");
    sub->setStyleSheet("font-size:14px;color:" C_MUT ";"); sub->setAlignment(Qt::AlignCenter);
    vb->addWidget(sub); vb->addSpacing(24);

    // Booking reference box
    auto* refBox=new QFrame; refBox->setFixedWidth(460);
    refBox->setStyleSheet("QFrame{background:#0d1f0d;border:1.5px solid #166534;border-radius:12px;}");
    auto* rbVb=new QVBoxLayout(refBox); rbVb->setContentsMargins(24,18,24,18); rbVb->setSpacing(4);
    auto* refLbl=new QLabel("REFERINȚĂ REZERVARE");
    refLbl->setStyleSheet("font-size:10px;font-weight:700;color:#4ade80;letter-spacing:2px;");
    refLbl->setAlignment(Qt::AlignCenter); rbVb->addWidget(refLbl);
    lblDoneRef=new QLabel("—");
    lblDoneRef->setStyleSheet("font-size:26px;font-weight:800;color:#22c55e;letter-spacing:4px;");
    lblDoneRef->setAlignment(Qt::AlignCenter); rbVb->addWidget(lblDoneRef);
    vb->addWidget(refBox,0,Qt::AlignHCenter); vb->addSpacing(20);

    // Summary card
    lblDoneSummary=new QLabel;
    lblDoneSummary->setTextFormat(Qt::RichText);
    lblDoneSummary->setFixedWidth(460);
    lblDoneSummary->setStyleSheet("background:" C_CARD ";color:" C_TXT ";"
                                  "border:1px solid " C_BOR2 ";border-radius:12px;"
                                  "padding:20px 24px;font-size:13px;");
    lblDoneSummary->setAlignment(Qt::AlignLeft|Qt::AlignTop);
    lblDoneSummary->setWordWrap(true);
    vb->addWidget(lblDoneSummary,0,Qt::AlignHCenter); vb->addSpacing(28);

    auto* bHome=redBtn("← Înapoi la Meniu Principal",50); bHome->setFixedWidth(280);
    connect(bHome,&QPushButton::clicked,this,[this]{ goTo(P_MAIN); });
    vb->addWidget(bHome,0,Qt::AlignHCenter);
    return w;
}
