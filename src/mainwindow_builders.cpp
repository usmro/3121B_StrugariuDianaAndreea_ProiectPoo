...
QRect SeatMapWidget::seatRect(int r,int c) const{int s=seatSize(),g=seatGap(),ml=marginL(),mt=marginT();return{ml+c*(s+g),mt+r*(s+g),s,s};}
...
QSize SeatMapWidget::sizeHint() const{
    ...
    int s=seatSize(),g=seatGap();
    return{marginL()+sala->getColoane()*(s+g)-g+24,marginT()+sala->getRanduri()*(s+g)-g+28};
}
...
void SeatMapWidget::paintEvent(QPaintEvent*){
    ...
    int s=seatSize(),g=seatGap(),ml=marginL(),mt=marginT();
    ...
    p.setPen(QColor(C_DIM)); p.setFont(QFont("Segoe UI",qMax(7,s/6)));
    ...
    for(int r=0;r<sala->getRanduri();r++){
        ...
        for(int c=0;c<cols;c++){
            ...
            if(!lbl.isEmpty()){p.setPen(txtC);p.setFont(QFont("Segoe UI",qMax(6,s/6),isSel?QFont::Bold:QFont::Normal));p.drawText(sr,Qt::AlignCenter,lbl);}
        }
    }
}
...
MenuCard::MenuCard(const QPixmap& icon,const QString& title,const QString& desc,bool gold,QWidget* parent)
    ...
    if(gold){nor_="QFrame{background:#16120a;border:1.5px solid #3a2a0a;border-left:3px solid " C_GOLD ";border-radius:0 14px 14px 0;}";
             hov_="QFrame{background:#1e180d;border:1.5px solid " C_GOLD ";border-left:3px solid " C_GOLD ";border-radius:0 14px 14px 0;}";}
    else    {nor_="QFrame{background:" C_CARD ";border:1.5px solid " C_BOR2 ";border-left:3px solid " C_RED ";border-radius:0 14px 14px 0;}";
...
LocationCard::LocationCard(const CinemaLocation& loc,bool isCurrentCity,QWidget* parent)
    ...
    setMinimumHeight(90);setMaximumHeight(116);setCursor(Qt::PointingHandCursor);
...
MainWindow::MainWindow(QWidget* parent):QMainWindow(parent){
    setWindowTitle(QString("Cinema Lumi") + QChar(0x00E8) + QString("re"));setMinimumSize(800,560);
    {
...
static QWidget* pageWrap(QVBoxLayout*& out){
    ...
    out=new QVBoxLayout(w);out->setContentsMargins(20,16,20,16);out->setSpacing(0);return w;
}
...
QWidget* MainWindow::buildLoginPage(){
    ...
    auto* card=new QFrame;card->setMaximumWidth(430);card->setMinimumWidth(320);
...
QWidget* MainWindow::buildMainPage(){
    ...
    heroFrame=new QFrame;heroFrame->setFixedHeight(140);
...
QWidget* MainWindow::buildReserveLocPage(){
    ...
    seatMapWidget=new SeatMapWidget;
seatMapWidget->setSizePolicy(QSizePolicy::Expanding,QSizePolicy::Expanding);
iVb->addWidget(seatMapWidget,0,Qt::AlignHCenter);
    scroll->setWidget(inner);vb->addWidget(scroll,1);
...
QWidget* MainWindow::buildMapPage(){
    ...
    mapWidget=new SeatMapWidget;mapWidget->readOnly=true;
mapWidget->setSizePolicy(QSizePolicy::Expanding,QSizePolicy::Expanding);
iVb->addWidget(mapWidget,0,Qt::AlignHCenter);
    scroll->setWidget(inner);vb->addWidget(scroll,1);
