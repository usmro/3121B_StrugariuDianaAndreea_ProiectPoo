...
MenuCard::MenuCard(const QString& emoji, const QString& title, const QString& desc, bool gold, QWidget* parent)
    ...
    if (gold) {
        nor_ = "QFrame{background:#16120a;border:1.5px solid #3a2a0a;border-radius:14px;}";
        hov_ = "QFrame{background:#1e180d;border:2px solid " C_GOLD ";border-radius:14px;}";
    }
    else {
        nor_ = "QFrame{background:" C_CARD ";border:1.5px solid " C_BOR2 ";border-radius:14px;}";
...
void MainWindow::goTo(Page p) {
    ...
    else if (p == P_RESERVE_SALA)   stepBar->setStep(2); // now "Programare"
...
MainWindow::MainWindow(QWidget* parent) :QMainWindow(parent) {
    ...
    stack->addWidget(buildReserveShowingPage()); // 4  P_RESERVE_SALA → now "Programare"
    stack->addWidget(buildReserveDataPage());    // 5  P_RESERVE_DATA
    stack->addWidget(new QWidget());             // 6  P_RESERVE_ORA  → unused placeholder
...
QWidget* MainWindow::buildNavbar() {
    ...
    connect(btnLoc, &QPushButton::clicked, this, [this] {
        // Keep user logged in, just go back to location selection
...
QWidget* MainWindow::buildMapPage() {
    ...
    scroll->setWidget(inner);vb->addWidget(scroll, 1);
    // Auto-refresh: sala/date change → repopulate ora → refresh map
...
QWidget* MainWindow::buildProfilePage() {
    ...
    connect(bPw, &QPushButton::clicked, this, [this] {
        ...
        if (!ok || old.toStdString() != u->parola) { showToast("Parolă incorectă!");return; }
        QString nw = QInputDialog::getText(this, "Parolă nouă", "Parola nouă:", QLineEdit::Password, "", &ok);if (!ok || nw.isEmpty())return;
        QString cf = QInputDialog::getText(this, "Confirmare", "Confirmă parola:", QLineEdit::Password, "", &ok);if (!ok || nw != cf) { showToast("Parolele nu coincid!");return; }
        cinema.schimbaParola(u->username, nw.toStdString());u->parola = nw.toStdString();showToast("Parolă schimbată!", false);
        });
    connect(bk, &QPushButton::clicked, this, [this] {goTo(P_MAIN);});return w;
}
