...
void MainWindow::onLocationSelected(const QString& id){
    ...
    if(!loc) return;
    lblNavLocation->setText("📍 "+QString::fromStdString(loc->oras)+" · "+QString::fromStdString(loc->nume));
    refreshMainMenu();
    goTo(P_MAIN);
}
...
void MainWindow::refreshFilmMiniHeaders(){
    ...
    if(miniSala) miniSala->setText(t);if(miniData) miniData->setText(t);
    if(miniOra) miniOra->setText(t);if(miniLoc) miniLoc->setText(t);
    if(miniFood) miniFood->setText(t);if(miniConf) miniConf->setText(t);
}
...
void MainWindow::refreshHeroBanner(){
    ...
    Film* f=active[heroIdx];lblHeroTitle->setText(QString::fromStdString(f->getTitlu()));
...
void MainWindow::refreshMapPage(){
    ...
    auto* ore=cinema.getOreCurente();if(!ore||oi>=(int)ore->size())return;
    auto zile=urmatoroarele7Zile();if(di>=(int)zile.size())return;
    mapWidget->load(cinema.getSalaByIdSafe((*sali)[si].getId()),zile[di],(*ore)[oi]);
}
void MainWindow::refreshCancelSeats(){
    ...
    auto* ore=cinema.getOreCurente();if(!ore||oi>=(int)ore->size())return;
    auto zile=urmatoroarele7Zile();if(di>=(int)zile.size())return;
    cancelWidget->load(cinema.getSalaByIdSafe((*sali)[si].getId()),zile[di],(*ore)[oi]);
...
void MainWindow::refreshAdminOre(){
    lwAdminOre->clear();
    auto* ore=cinema.getOreCurente();if(!ore)return;
    for(auto&o:*ore) lwAdminOre->addItem(QString::fromStdString(o));
}
...
void MainWindow::refreshFoodPage(){
    // Reset all qty labels and food order
    rezFood=FoodOrder{};
...
void MainWindow::onRegister(){
    ...
    if(cinema.inregistrare(usr.toStdString(),par.toStdString())){
        QMessageBox::information(this,"","Cont creat! Bun venit, "+usr+"!");
...
void MainWindow::onHartaSala(){
    ...
    auto* ore=cinema.getOreCurente();if(ore) for(auto&o:*ore) cbMapOra->addItem(QString::fromStdString(o));
...
void MainWindow::onAnulareRezervare(){
    ...
    auto* ore=cinema.getOreCurente();if(ore) for(auto&o:*ore) cbCancelOra->addItem(QString::fromStdString(o));
...
void MainWindow::onProceedFromDetail(){
    if(!rezFilm)return;
    lwSali->clear();
    auto* sali=cinema.getSaliCurente();if(!sali)return;
    for(auto&s:*sali){
...
void MainWindow::onSalaSelected(){
    ...
    if(idx<0||!sali||(int)sali->size()<=idx){QMessageBox::warning(this,"","Selectează o sală!");return;}
    rezSalaId=(*sali)[idx].getId();goTo(P_RESERVE_DATA);
}
void MainWindow::onDataSelected(){
    int idx=lwData->currentRow();if(idx<0){QMessageBox::warning(this,"","Selectează o dată!");return;}
...
void MainWindow::onOraSelected(){
    int idx=lwOra->currentRow();if(idx<0){QMessageBox::warning(this,"","Selectează o oră!");return;}
    auto* ore=cinema.getOreCurente();if(!ore)return;
    rezOra=(*ore)[idx];
    Sala* sala=cinema.getSalaByIdSafe(rezSalaId);
    rezLocuri.clear();
...
void MainWindow::onLocSelected(int row,int col){
    ...
    if((int)rezLocuri.size()>=maxLocuri){QMessageBox::warning(this,"","Maxim 8 locuri per rezervare!");return;}
...
void MainWindow::onConfirmReservation(){
    ...
    try {
        ...
        QMessageBox::critical(this,"Eroare rezervare",QString::fromStdString(e.what()));return;
    } catch(const SalaNotFoundException& e){
        QMessageBox::critical(this,"Eroare rezervare",QString::fromStdString(e.what()));return;
    } catch(const std::exception& e){
        QMessageBox::critical(this,"Eroare",QString::fromStdString(e.what()));return;
    }
    // Extract reference
...
void MainWindow::onCancelReservation(){
    ...
    int si=cbCancelSala->currentIndex(),di=cbCancelData->currentIndex(),oi=cbCancelOra->currentIndex();
    auto* sali=cinema.getSaliCurente();auto* ore=cinema.getOreCurente();
    if(!sali||!ore||si<0||di<0||oi<0)return;
    ...
    cancelRow=-1;cancelCol=-1;refreshCancelSeats();QMessageBox::information(this,"","Rezervare anulată cu succes!");
}
...
void MainWindow::onAdminAddFilm(){
    ...
    refreshAdminFilms();refreshUpcomingStrip();QMessageBox::information(this,"","Film adăugat!");
}
...
void MainWindow::onAdminAddOra(){
    bool ok;QString ora=QInputDialog::getText(this,"Adaugă Oră","Ora (HH:MM):",QLineEdit::Normal,"",&ok);
...
void MainWindow::onAdminDeleteOra(){
    ...
    auto* ore=cinema.getOreCurente();if(!ore)return;
    if(QMessageBox::question(this,"Confirmare","Ștergi ora "+QString::fromStdString((*ore)[idx])+"?")!=QMessageBox::Yes)return;
