...
// Helper: get unique hours from schedule for a sala on a date
...
void MainWindow::refreshSearchResults() {
    ...
    // Only show films available at the current cinema location
    auto filmeLocatie = cinema.getFilmePentruLocatie();
...
void MainWindow::onHartaSala() {
    ...
    // Ora combo updated dynamically when sala+date selected
...
void MainWindow::onDataSelected() {
    ...
    for (const Programare* p : programari) {
        ...
        // Card is a QPushButton so it handles clicks natively
        ...
        // Info block (sala name + seats text — no child-parented frame trick)
