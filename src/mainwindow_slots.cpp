...
void MainWindow::populateFilmDetail(Film* f) {
    ...
    if (!f->getPosterPath().empty()) {
        ...
        if (!px.isNull()) { detailPoster->setPixmap(px.scaled(220, 330, Qt::KeepAspectRatioByExpanding, Qt::SmoothTransformation));detailPoster->setStyleSheet("border:1.5px solid #e50914;border-radius:12px;"); }
    ...
    connect(reply, &QNetworkReply::finished, this, [this, reply] {
        ...
        connect(imgReply, &QNetworkReply::finished, this, [this, imgReply] {
            ...
            if (!px.isNull()) {
                ...
                detailPoster->setStyleSheet("border:1.5px solid #e50914;border-radius:12px;");
...
void MainWindow::refreshSearchResults() {
    ...
    for (auto* fp : filmeLocatie) {
        ...
        auto* item = new QListWidgetItem(lwFilms);item->setSizeHint(QSize(0, 88));
        ...
        auto* rcw = new QWidget;rcw->setFixedWidth(110);rcw->setStyleSheet("background:transparent;");
        auto* rc = new QVBoxLayout(rcw);rc->setSpacing(2);rc->setAlignment(Qt::AlignRight | Qt::AlignVCenter);
        QString pretTxt = QString::number((int)f.getPretBaza()) + " RON";
        if (f.getEste3D()) pretTxt += "  <span style='color:#38bdf8;font-size:11px;'>3D</span>";
        auto* pl = new QLabel(pretTxt);pl->setTextFormat(Qt::RichText);
...
void MainWindow::refreshAdminReport() {
    ...
    while (std::getline(f, ln)) {
        ...
        if (film.empty())continue;double pret = 0;try { pret = std::stod(sp); }
        catch (...) {}
