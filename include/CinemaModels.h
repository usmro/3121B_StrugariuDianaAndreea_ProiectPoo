...
struct CinemaLocation {
    ...
    int         minuteDeschidere  = 10*60;  

    int         minuteInchidere   = 22*60+30; 

    bool        program24h        = false;
    std::vector<Sala>       sali;
    std::vector<Programare> program; 
...
struct PreferintaLoc {
    CategorieLocSeat tip    = CategorieLocSeat::STANDARD;
    int              rand_  = -1;  
...
struct ContUtilizator {
    ...
    std::map<std::string,int> evaluari;    
