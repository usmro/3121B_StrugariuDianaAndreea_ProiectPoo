...
private:
    ...
    std::vector<CinemaLocation> locatii;     
...
public:
            ...
            if (sscanf(ln.c_str()+pLoc,"Loc: R%d L%d",&r,&l)==2)
                for (auto& s : locatieCurenta->sali)
                    if (s.getNume()==salaNume)
                        try { s.ocupaLoc(d,oraStr,r-1,l-1); }
                        catch(...){} 
    ...
    bool login(const std::string& usr, const std::string& par) {
        ...
        curent=u; incarcaIstoricUser(*curent);
        incarcaEvaluari(*curent);
        curent->preferinta = incarcaPreferinta(curent->username);
        return true;
    }
    ...
    std::string fisierRatings(const std::string& usr) { return "data/ratings_"+usr+".txt"; }
    std::string fisierPreferinte(const std::string& usr) { return "data/preferinte_"+usr+".txt"; }

    void salveazaEvaluari(const ContUtilizator& u) {
        std::ofstream f(fisierRatings(u.username), std::ios::trunc);
        for(auto& [titlu,stele]:u.evaluari) f<<titlu<<"|"<<stele<<"\n";
    }
    void incarcaEvaluari(ContUtilizator& u) {
        std::ifstream f(fisierRatings(u.username)); if(!f.is_open()) return;
        std::string ln;
        while(std::getline(f,ln)){
            if(!ln.empty()&&ln.back()=='\r') ln.pop_back();
            size_t p=ln.rfind('|'); if(p==std::string::npos) continue;
            std::string titlu=ln.substr(0,p); int stele=0;
            try{stele=std::stoi(ln.substr(p+1));}catch(...){}
            if(stele>=1&&stele<=5) u.evaluari[titlu]=stele;
        }
    }
    

    bool aVazutFilm(const ContUtilizator& u, const std::string& titlu) const {
        for(auto& r:u.istoricRezervari)
            if(r.find(titlu)!=std::string::npos) return true;
        return false;
    }
    void evalueazaFilm(const std::string& titlu, int stele) {
        if(!curent||stele<1||stele>5) return;
        curent->evaluari[titlu]=stele;
        salveazaEvaluari(*curent);
        

        double sum=0; int cnt=0;
        for(auto& u:utilizatori){
            ContUtilizator tmp=u; incarcaEvaluari(tmp);
            auto it=tmp.evaluari.find(titlu);
            if(it!=tmp.evaluari.end()){sum+=it->second;cnt++;}
        }
        if(cnt>0) for(auto& f:filme) if(f.getTitlu()==titlu){
            f.setRating(sum/cnt); salveazaFilme(); break;
        }
    }
    int getRatingUtilizator(const std::string& titlu) const {
        if(!curent) return 0;
        auto it=curent->evaluari.find(titlu);
        return it!=curent->evaluari.end()?it->second:0;
    }

    

    void salveazaPreferinta(const PreferintaLoc& p) {
        if(!curent) return;
        curent->preferinta=p;
        std::ofstream f(fisierPreferinte(curent->username),std::ios::trunc);
        f<<(int)p.tip<<"|"<<p.rand_<<"|"<<p.coloana<<"\n";
    }
    PreferintaLoc incarcaPreferinta(const std::string& usr) {
        PreferintaLoc p; std::ifstream f(fisierPreferinte(usr));
        if(!f.is_open()) return p;
        std::string ln; std::getline(f,ln);
        if(!ln.empty()&&ln.back()=='\r') ln.pop_back();
        std::stringstream ss(ln); std::string t,r,c;
        if(std::getline(ss,t,'|')&&std::getline(ss,r,'|')&&std::getline(ss,c,'|')){
            try{p.tip=(CategorieLocSeat)std::stoi(t);p.rand_=std::stoi(r);p.coloana=std::stoi(c);p.setat=true;}
            catch(...){}
        }
        return p;
    }
    PreferintaLoc getPreferintaCurenta() const {
        return curent ? curent->preferinta : PreferintaLoc{};
    }
    ...
    void genereazaProgramZi(CinemaLocation& loc, const Data& d) {
        ...
        for(auto& sala : loc.sali) {
            ...
            while(curent < inchidere) {
                ...
                filmIdx++; incercari++;
                if(incercari > (int)rotatie.size()*3) break; 


                int dur = f->getDurata();
                int sfarsit = curent + dur + 45; 
    ...
    void proceseazaRezervare(int idSala,const Data& data,
        ...
        double ziMult=multiplicatorZi(data); 
